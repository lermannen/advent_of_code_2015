...
def turn_on(coords, grid)
  (coords[0]..coords[2]).each do |row|
    (coords[1]..coords[3]).each do |col|
      grid[row][col] += 1
    end
  end
  grid
...
def turn_off(coords, grid)
  (coords[0]..coords[2]).each do |row|
    (coords[1]..coords[3]).each do |col|
      grid[row][col] -= 1 unless grid[row][col] < 1
    end
  end
  grid
end

def toggle(coords, grid)
  (coords[0]..coords[2]).each do |row|
    (coords[1]..coords[3]).each do |col|
      grid[row][col] += 2
    end
...
data.each_line do |line|
   handle_input(line, grid)
 end

p grid.reduce(0) { |sum, n| sum + n.reduce(:+) }
